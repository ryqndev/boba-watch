...
const MobileDashboard = () => {
    ...
    return (
        <main className={cn.wrapper}>
            <h4 className={clsx(cn.bw, cn.title)}>{t('Monthly Spending')}</h4>
            <Card id="chart-holder" className={cn['daily-chart']}>
                <div className={cn.description}>
                    {t('MONTHLY LIMIT')}: {t('$')}{toMoney(user.profile.budget, user.profile.budget/10000 > 1)}
                    <br />
                    <span>{t('$')}{toMoney(metrics.tc, metrics.tc/10000 > 1)}</span>
                    <br />
                    {t('REMAINING')}: {t('$')}{toMoney(user.profile.budget - metrics.tc)}
                </div>
                <UserSunburst budget={user.profile.budget} spent={metrics.tc} width={width}/>
            </Card>
            <Card className={cn.budget}>
                <p>{t('This is how much you’ve spent on drinks so far')}:</p>
                <h2 className={cn.bw}>{t('$')}{toMoney(metrics.ctc, metrics.ctc/10000 > 1)}</h2>
            </Card>
            <Card className={cn.limit} style={{backgroundPositionY: (100 - parseInt((metrics.td / user.profile.limit) * 100)) * 2.7}}>
                <h3 className={cn.bw}>{parseInt((metrics.td / user.profile.limit) * 100)}%</h3>
            ...
            <DailyHeatMap className={cn['daily-chart']} data={hourlyMetric} width={width} />
