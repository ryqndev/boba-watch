...
const NoDrinksLabel = ({label}) => {
    return (
        <div className="thaman-color" key={1}>
            <h3 className="bw">
                {label}
            </h3>
        </div>
...
const History = ({drinks}) => {
    const [monthly, setMonthly] = useState({drinks: [<NoDrinksLabel label="No drinks this month :(" />], sum: 0});
    const [complete, setComplete] = useState({drinks: [<NoDrinksLabel label="Add a drink to start!" />], sum: 0});
